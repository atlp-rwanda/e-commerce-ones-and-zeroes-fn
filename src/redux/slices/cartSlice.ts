...
export const addProductInCart = createAsyncThunk(
  ...
  async ({ productId, quantity }: { productId: string; quantity: number }) => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to add product in the cart";
...
export const fetchProductsInCart = createAsyncThunk(
  ...
  async () => {
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to fetch products in the cart";
...
export const fetchTotalInCart = createAsyncThunk(
  "cart/fetchtotalInCart",
  async () => {
    // Retrieve the token from local storage
    const token = localStorage.getItem("token");
    ...
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to fetch Total in the cart";
...
export const updateProductQuantityInCart = createAsyncThunk(
  ...
  async ({ productId, quantity }: { productId: string; quantity: number }) => {
    ...
    try {
      ...
      console.log(response.data);
      toast.success(`Cart Product was successfully updated`);
      return response.data;
    } catch (error: any) {
      const errorMessage =
        error.response?.data?.message || "Failed to update product quantity";
      console.error("Failed to update product quantity:", errorMessage);
      toast.error(`${errorMessage}`);
      throw error;
    }
  }
...
export const deleteProductInCart = createAsyncThunk(
  ...
  async ({ productId }: { productId: string }) => {
    ...
    try {
      ...
      console.log(response.data);
      toast.success(`Cart Product was successfully Removed`);
      return response.data;
    } catch (error: any) {
      const errorMessage =
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchProductsInCart.rejected, (state, action) => {
        state.loading = false;
        state.error =
          action.error.message || "Failed to fetch products in cart";
      })
